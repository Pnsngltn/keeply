...
def send_email(subject, recipients, body, html=None):
    ...
    if not app.config.get("MAIL_USERNAME") or not app.config.get("MAIL_PASSWORD"):
        current_app.logger.warning("Mail credentials not configured; skipping email.")
        return False
...
def register():
    ...
    if request.method == "POST":
        ...
        # Automatically log in new user and redirect to dashboard
...
def delete_timeslots():
    """Delete one or more timeslots owned by the logged-in user.

    Accepts JSON: { "slot_ids": [1,2,3] }
    Only timeslots belonging to the current user and not currently booked
    (and without appointments) will be deleted. Returns which were deleted
    and which were ignored.
    """
    data = request.json
    if not data or "slot_ids" not in data:
        return jsonify({"error": "Missing slot_ids"}), 400

    try:
        slot_ids = [int(x) for x in data["slot_ids"]]
    except Exception:
        return jsonify({"error": "Invalid slot_ids"}), 400

    if not slot_ids:
        return jsonify({"deleted": [], "not_deleted": []})
...
def api_book():
    ...
    try:
        ...
        if user_email:
            # Use the helper that sends asynchronously; best-effort only.
            send_email(
